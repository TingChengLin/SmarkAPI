...
class User
  module OmniauthCallbacks
    ["facebook", "evernote", "github"].each do |provider|
      define_method "find_or_create_for_#{provider}" do |response|
        ...
        authorization = Authorization.where("authorizations.provider" => provider , "authorizations.uid" => uid).first
        if authorization && user = authorization.user
          user
